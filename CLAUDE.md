...
**La tarjeta "Total cajas" lleva una leyenda de presentación** a la derecha del número grande (`.rfr-cajas-val-row` = flex con el `.kpi-val` y la leyenda), en dos renglones: `6 oz - N cajas` / `12 oz - N cajas`, más `otras - N cajas` solo si hay cajas con otra etiqueta (`s.cajas − s.c6oz − s.c12oz` en Por semana; acumulado inline desde `r.cajas` en Por día). La arma `rfrCajasLegHtml(c6, c12, cOtras)`, compartida por las dos vistas: cada renglón son **4 celdas de un `inline-grid`** (`.rfr-cajas-leg`) — etiqueta · guion · número · "cajas" — con etiqueta y número alineados a la derecha (`text-align:right`), para que guiones, números y "cajas" queden en columna entre renglones. Es a pedido del usuario — la excepción a la regla "sin leyendas por iniciativa propia". `rfrSemanaAcum()` ya acumulaba `c6oz`/`c12oz`; antes no se mostraban en ningún lado. Solo en Por día y Por semana.
...
- **Almacén: lote com = compra de comercializadora** (ago-2026, spec en `docs/superpowers/specs/2026-08-11-compra-almacen-comercializadora-design.md`). El lote lleva campos extra — `proveedor`, `precioCompra`, `fleteCompra`, `gastoIds`, más `compraId`/`kgPagar`/`descPct` — visibles solo cuando el Negocio es `com` (`onAlmLocChange()` muestra/oculta `#af-com-fields`; en jal/nay se ignoran al guardar). Persistencia dual cableada en los cuatro lugares (columnas `proveedor`/`precio_compra`/`flete_compra`/`gasto_ids`/`compra_id`/`kg_pagar`/`desc_pct` en `alm_lotes`).
  ...
  **Un despacho no se edita** (sep-2026): `editarEnvio()` lo rechaza y `verEnvio()` no pinta ✏️ Editar. Los dos formularios lo reescribían como captura propia —sin `loteIds` ni `esDespachoAlm`— y el de comercializadora exige proveedor, así que al guardar volvía a cargar la compra que ya vivía en el lote: segunda cuenta por pagar y segundo gasto de mercancía y flete. Pasó con `Comer/2026/008` (Pollo).
