...
**El proceso va primero a propósito.** Chart.js arranca el primer segmento a las 12 en punto, así que los defectos —lo accionable— quedan arriba y los kg buenos cierran el anillo.

**El tramo de Buenos va despegado** del resto con `RFR_OFFSET_BUENOS` (16 px), pasado como arreglo en `dataset.offset` — una entrada por rebanada, con el valor solo en el índice `d.idxBuenos` que devuelve `rfrDonaCalidad()`. El `layout.padding` del cfg vale `RFR_OFFSET_BUENOS + 4` en los cuatro lados para que ese desplazamiento no lo recorte el borde del canvas; si se sube el offset hay que subir el padding con él.
...
- Colores (`DEF_COLS_RFR`, definidos por el usuario): sobremadura vino `#66232e`, deforme azul `#2a78d6`, irregular amarillo `#eda100`, desgrane naranja `#f5964a`, hongos gris `#888780`; y `RFR_COL_BUENOS` es `#ea3155`. Esa constante alimenta tanto la dona como los puntitos de las filas de defectos, así que cambiarla actualiza los dos a la vez.
