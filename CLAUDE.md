...
**Leyenda propia (`rfrDonaLeyendaHtml`).** La nativa de Chart.js va desactivada (`legend.display:false`) porque sigue el orden de los datos —donde el proceso va primero— y no deja apartar un item del resto. La propia pone **"kg bueno" al inicio, pegado a la izquierda y separado por un borde**, aunque en el anillo ese tramo vaya al final; `d.idxBuenos` es lo que permite sacarlo de su posición sin tocar el orden de la dona. Se inserta como hermano del contenedor del canvas en las tres vistas — en Por semana y Por ciclo el canvas va envuelto en un flex column para que la leyenda quede debajo y no al lado de las filas de defectos.
...
- Colores (`DEF_COLS_RFR`, definidos por el usuario): sobremadura vino `#66232e`, deforme morado `#7E57C2`, irregular mostaza `#C9A227`, desgrane naranja `#E67E22`, hongos gris `#888780`; y `RFR_COL_BUENOS` es verde `#1baf7a`, con etiqueta `RFR_LBL_BUENOS` = "kg bueno". Esa constante alimenta tanto la dona como los puntitos de las filas de defectos, así que cambiarla actualiza los dos a la vez.
